...
void Map::Clear(Room* pRoom, Room* pPrev)
{
	while (m_pHead != 0)
	{
		...
		{
			for (short dirIdx = 4;dirIdx != static_cast<short>(e_Directions::NONE);--dirIdx)
			{
                e_Directions dir = static_cast<e_Directions>(dirIdx);
				if (pRoom->GetDirection(dir) != 0 && pRoom->GetDirection(dir) != pPrev)
				{
					Room* pTemp = pRoom->GetDirection(dir);
					if (ChkDeadEnd(pTemp))
					{
						pRoom->SetDirection(dir, 0);
						pPrev = pRoom;
						pRoom = pTemp;
						break;
					}
...
bool Map::ChkDeadEnd(Room* pRoom)
{
	...
	for (short i = 0; i < static_cast<short>(e_Directions::NONE); --i)
	{
        e_Directions dir = static_cast<e_Directions>(i);
		if (pRoom->GetDirection(dir) != 0)
		{
			++counter;
		}
	}
...
void Map::AddRoom(std::vector<e_Directions> dirs, std::string name)
{
	if (m_pHead == 0)
	{
	...
	{
		Room* pTemp = m_pHead;
		for (std::vector<e_Directions>::const_iterator iter = dirs.begin(); iter != dirs.end(); ++iter)
		{
			if (iter != dirs.end() - 1)
			{
...
void Map::RemoveRoom(std::vector<e_Directions> dirs)
{
	if (dirs[0] == e_Directions::NONE)
	{
		delete m_pHead;
	}
	else
	{
		...
		for (std::vector<e_Directions>::const_iterator iter = dirs.begin(); iter != dirs.end(); ++iter)
		{
			if (pTemp->GetDirection(*iter) != nullptr)
			{
				pTemp = pTemp->GetDirection(*iter);
			}
			else
		...
		{
            e_Directions dir = static_cast<e_Directions>(i);
			if (pTemp->GetDirection(dir) != 0)
			{
				pTemp2 = pTemp->GetDirection(dir);
				pTemp2->SetDirection(OppDir(dir), 0);
			}
		}
		delete pTemp;
	}
...
void Map::RandGen(std::vector<e_Directions> dirs, e_Directions prevDir, short depth)
{
	std::string name;
	...
	if (size < 8)
	{
		...
		else
		{
			...
			if (randRooms == 0)
			{
				...
				while (randNum == static_cast<short>(prevDir))
				{
					randNum = rand() % 4;
				}
                ++randNum;
				dirs.push_back(static_cast<e_Directions>(randNum));
				AddRoom(dirs, name);
				++size;
				prevDir = OppDir(static_cast<e_Directions>(randNum));
				if (depth < 3)
				{
			...
			else if (randRooms == 1)
			{
				...
				while (randNum0 == static_cast<short>(prevDir) || randNum1 == static_cast<short>(prevDir) || randNum0 == randNum1)
				{
					randNum0 = rand() % 4;
					randNum1 = rand() % 4;
				}
                ...
                ++randNum1;
				size += 2;
				dirs.push_back(static_cast<e_Directions>(randNum0));
				AddRoom(dirs, name);
				prevDir = OppDir(static_cast<e_Directions>(randNum0));
				if (depth < 3)
				{
					RandGen(dirs, prevDir, ++depth);
				}
				dirs.pop_back();
				name = GiveRandName();
				dirs.push_back(static_cast<e_Directions>(randNum1));
				AddRoom(dirs, name);
				prevDir = OppDir(static_cast<e_Directions>(randNum1));
				if (depth < 3)
				{
					RandGen(dirs, prevDir, ++depth);
				}
			...
			else if (randRooms == 2)
			{
				...
				while (static_cast<e_Directions>(randNum0) == prevDir || static_cast<e_Directions>(randNum1) == prevDir || static_cast<e_Directions>(randNum2) == prevDir || randNum0 == randNum1 || randNum0 == randNum2 || randNum1 == randNum2)
				{
					randNum0 = rand() % 4;
					randNum1 = rand() % 4;
					randNum2 = rand() % 4;
				...
				dirs.pop_back();
				name = GiveRandName();
				dirs.push_back(static_cast<e_Directions>(randNum2));
				AddRoom(dirs, name);
				prevDir = OppDir(static_cast<e_Directions>(randNum2));
				if (depth < 2)
				{
					RandGen(dirs, prevDir, ++depth);
				}
			...
			{
				size += 4;
				dirs.push_back(e_Directions::NORTH);
				AddRoom(dirs, name);
				prevDir = OppDir(e_Directions::NORTH);
				if (depth < 2)
				{
					RandGen(dirs, prevDir, ++depth);
				}
				dirs.pop_back();
				name = GiveRandName();
				dirs.push_back(e_Directions::EAST);
				AddRoom(dirs, name);
				prevDir = OppDir(e_Directions::EAST);
				if (depth < 2)
				{
					RandGen(dirs, prevDir, ++depth);
				}
				dirs.pop_back();
				name = GiveRandName();
				dirs.push_back(e_Directions::SOUTH);
				AddRoom(dirs, name);
				prevDir = OppDir(e_Directions::SOUTH);
				if (depth < 2)
				{
					RandGen(dirs, prevDir, ++depth);
				}
				dirs.pop_back();
				name = GiveRandName();
				dirs.push_back(e_Directions::WEST);
				AddRoom(dirs, name);
				prevDir = OppDir(e_Directions::WEST);
				if (depth < 2)
				{
					RandGen(dirs, prevDir, ++depth);
				}
...
Room::e_Directions Map::OppDir(e_Directions dir)
{
    typedef Room::e_Directions e_Directions;
	e_Directions dirs[4] = {e_Directions::NORTH, e_Directions::EAST, e_Directions::SOUTH, e_Directions::WEST};
	//std::string oppDirs[4] = {"SOUTH", "WEST", "NORTH", "EAST"};
    e_Directions oppDirs[4] = {e_Directions::SOUTH,e_Directions::WEST,e_Directions::NORTH,e_Directions::EAST};
	for (short i = 0; i < 4; ++i)
	{
		if (dir == dirs[i])
		{
